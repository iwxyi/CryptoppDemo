...
int scoped_main(int argc, char *argv[])
{
    // 这里是测试文本
    try {
        QString plain = "1234512345";
        QString key = "keykeykeykey";
        qDebug() << (plain = QString::fromStdString(EncryptString(plain.toUtf8().data(), key.toUtf8().data())));
        plain += "2"; // 加一点点居然没事？
        qDebug() << (plain = QString::fromStdString(DecryptString(plain.toUtf8().data(), key.toUtf8().data())));
    }
    catch(const Exception &e)
    {
        std::cout << "\n出现错误: " << e.what() << std::endl;
    }
